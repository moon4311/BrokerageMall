...
public class EgovFileUploadUtil extends EgovFormBasedFileUtil {
    ...
    private final static String extWhiteList = EgovProperties.getProperty("Globals.fileUpload.Extensions");
    private final static long maxFileSize = Long.parseLong(EgovProperties.getProperty("Globals.fileUpload.maxSize"));   //업로드 최대 사이즈 설정 (100M)
...
//			vo.setServerSubPath(getTodayString());
			vo.setServerSubPath(where+SEPERATOR+getTodayString());
			...
			if (mFile.getSize() > 0) {
				...
				try {
					...
					saveFile(is, new File(EgovWebUtil.filePathBlackList(uploadDir+SEPERATOR+ vo.getServerSubPath() + SEPERATOR + vo.getPhysicalName())));
				} finally {
					if (is != null) {
						is.close();
					}
	...
	/**
	 * @param request
	 * @param where 첨부파일 폴더의 하위 폴더명
	 * @return
	 * @throws Exception
	 */
	public static List<EgovFormBasedFileVo> uploadFilesExt(HttpServletRequest request, String where) throws Exception {
		return uploadFilesExt(request,where,maxFileSize,extWhiteList);
	}
